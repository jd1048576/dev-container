FROM docker.io/library/ubuntu:26.04@sha256:e153663f92c94118ff22a5dc397b59b351ffd695480566debb5850e017e5937a
ARG TARGETPLATFORM
...
RUN --mount=type=cache,target=/var/lib/apt,sharing=locked --mount=type=tmpfs,target=/var/log set -eux; \
  ...
  printf "Types: deb\nURIs: https://download.docker.com/linux/ubuntu\nSuites: %s\nComponents: stable\nArchitectures: %s\nSigned-By: %s\n" "$(. /etc/os-release && echo "${UBUNTU_CODENAME:-$VERSION_CODENAME}")" "$(dpkg --print-architecture)" "${GPG_KEY_FILE}" | tee /etc/apt/sources.list.d/docker.sources; \
  apt-get -qq update; \
  apt-get -qqy --no-install-recommends install \
...
RUN --mount=type=cache,target=/var/lib/apt,sharing=locked --mount=type=tmpfs,target=/var/log set -eux; \
  ...
  printf "%s" "${GPG_KEY}" | base64 -d | gpg --batch --dearmor --output "${GPG_KEY_FILE}"; \
  printf "Types: deb\nURIs: https://pkgs.k8s.io/core:/stable:/v%s/deb\nSuites: /\nSigned-By: %s\n" "${KUBERNETES_MAJOR_MINOR_VERSION}" "${GPG_KEY_FILE}" | tee /etc/apt/sources.list.d/kubernetes.sources; \
  apt-get -qq update; \
  apt-get -qqy --no-install-recommends install \
...
ARG VSCODE_SERVER_VERSION="4.125.0"
...
RUN --mount=type=tmpfs,target=/root/.config --mount=type=tmpfs,target=/root/.local set -eux; \
  ...
  rm bundle.tar.gz; \
  ln -s "${VSCODE_SERVER_HOME}/lib/vscode/bin/remote-cli/code-server" /usr/local/bin/code; \
  code --version;
ENV EDITOR="/usr/local/bin/code --wait"
ENV GIT_EDITOR="/usr/local/bin/code --wait"
ENV VISUAL="/usr/local/bin/code --wait"
